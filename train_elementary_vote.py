...
def load_all(data_path: str) -> pd.DataFrame:
    ...
    if not dfs:
        raise FileNotFoundError(f"未找到 train/val/test 或 combined.csv 于 {data_path}")
    df = pd.concat(dfs, ignore_index=True)
    print(f"[INFO] 合并 train/val/test, 样本数 {len(df)}")
    return df


def prepare_splits(data_path: str, out_root: str, n_folds: int, test_size: float, seed: int):
...
def main(argv=None):
    ...
    parser.add_argument('--data_path', required=True, help='原始数据目录（含 train/val/test 或 combined.csv）')
    parser.add_argument('--output_dir', required=True, help='输出根目录（将生成 fold_*/train.csv 等）')
